...
function createData(name: string, amount: number, loans: any): Data {
  return {
    name,
    amount,
    loans,
...
export default function EnhancedTable() {
  ...
  const [loans, setLoans] = React.useState<loansData[]>([]);
  ...
  const fetchData = async () => {
    const getBank = await axios
      ...
      .then(async function (response) {
        ...
        const getAccounts = await axios
          .post("http://localhost:3001/accounts/view", {
        ...
        const getLoans = await axios
          .post("http://localhost:3001/loans/view", {
            bankId: response.data[0].bankId,
          })
          .then(function (response) {
    ...
    <div>
      ...
      >
        ...
        >
          ...
          <Link style={{ color: "#808080" }} href="/UpdateBank" color="inherit">
            Update Bank
          </Link>
        </Box>
        <div
        ...
        >
          <Card sx={{ maxWidth: 345, width: "45vw" }}>
            <CardActionArea>
              <CardContent>
                <Typography
                  gutterBottom
                      ...
                      .map((row, index) => {
                        ...
                        return (
                          <TableRow
                            hover
                            onClick={(event) =>
                              handleClick(event, String(row.name))
                            }
                            role="checkbox"
                          ...
                          >
                            ...
                            <TableCell align="left">{row.loans}</TableCell>
                          </TableRow>
                        );
                      })}
                    {emptyRows > 0 && (
